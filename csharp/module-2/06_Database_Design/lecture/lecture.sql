...
-- switch to using the system datebase
USE master;
...
-- in order to add tables, etc to the ArtGallery DB, i need to be using the ArtGallery DB
USE ArtGallery;
GO
...
CONSTRAINT fk_artwork FOREIGN KEY (artist_id) REFERENCES artist(artist_id)
...
CONSTRAINT pk_artwork_customer PRIMARY KEY (customer_id, artwork_id), --compostite primary key based on two columns
