        ...
        public Transfer AddTransfer(NewTransfer transfer, int fromAcctId, int toAcctId)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("INSERT INTO transfer (transfer_type_id, transfer_status_id, account_from, account_to, amount) VALUES (@transferType, @transferStatus, @accountFrom, @accountTo, @amount)", conn);
        ...
        public List<Transfer> GetTransfersForUser(int userId)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string sql = SQL_SelectTransfer + "WHERE (account_from IN (SELECT account_id FROM account WHERE user_id = @userId) OR account_to IN (SELECT account_id FROM account WHERE user_id = @userId))";
        ...
        public List<Transfer> GetPendingTransfersForUser(int userId)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string sql = SQL_SelectTransfer + "WHERE transfer_status_id = 1 AND account_from IN (SELECT account_id FROM account WHERE user_id = @userId)";
        ...
        private bool ChangeTransferStatus(int transferId, TransferStatus status)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("UPDATE transfer SET transfer_status_id = @transferStatus WHERE transfer_id = @transferId", conn);
        ...
        private bool TransferMoney(decimal amount, int acctFrom, int acctTo)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    @"UPDATE account SET balance = (balance - @amount) WHERE account_id = @accountFrom; 
                      UPDATE account SET balance = (balance + @amount) WHERE account_id = @accountTo", conn);
