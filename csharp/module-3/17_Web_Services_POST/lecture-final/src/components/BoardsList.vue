...
export default {
  ...
  methods: {
    ...
    saveNewBoard() {
        this.isLoading = true;
        this.showAddBoard = false; // take away the form so the user can't click the 'save' button 823,492 times while waiting for the Promise to resolve
        boardsService.addBoard(this.newBoard).then(() => {
          ...
          if(error.response){
            ...
            this.errorMsg = 'Error adding a new board, response recieved from the server was ' + error.response.statusText + '.';
          }
